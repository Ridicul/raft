 ...
 * and when successful, will call `snapshot_async` by means of the
 * `io->async_work` method. The callback to `io->async_work` will in turn call
 * `snapshot_finalize` in the main loop when the work has completed independent
 * of the return value of `snapshot_async`. An implementation that does not use
 * asynchronous snapshots MUST set `snapshot_async` to NULL. All memory
 * allocated by the snapshot routines MUST be freed by the snapshot routines
 * themselves.
 */
struct raft_fsm
{
...
struct raft_progress
{
    ...
    raft_time last_recv_time; /* TODO Timestamp of last received
                                 AppendEntriesResult */
};
...
struct raft
{
     ...
     *
     * 2. #configuration_index is non-zero while
     * #configuration_uncommitted_index is zero. In this case the content of
     * #configuration must match the one of the log entry at
     * #configuration_index.
     *
...
RAFT_API int raft_add(struct raft *r,
                      ...
                      const char *address,
                      raft_change_cb cb);
RAFT_API int raft_add_logger(struct raft *r,
                             struct raft_change *req,
                             raft_id id,
                             const char *address,
                             raft_change_cb cb);
