...
int recvAppendEntriesResult(struct raft *r,
    ...
    const struct raft_append_entries_result *result)
        {
    if(id == 4) printf("leader recv logger appendEntriesResult\n");
    int match;
    ...
    //TODO 更新 progress 的 last_recv_time
    for(int i=0;i<r->configuration.n;i++){
        if(r->configuration.servers[i].id == server->id){
            // i 即是该progress的下标
            raft_time now = r->io->time(r->io);
            r->leader_state.progress[i].last_recv_time = now;
            break;
        }
    }
