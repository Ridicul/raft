                             ...
                             const raft_term prev_term)
{
    ...
    //    printf("leader %d send %u entries starting at %llu to server %llu
    //    (last index %llu)\n",
    //           r->id, args->n_entries, args->prev_log_index, server->id,
    //           logLastIndex(&r->log));
    ...
    req->send.data = req;
    if (i == 3)
        printf("r->io->send message type %d\n", message.type);
...
int replicationProgress(struct raft *r, unsigned i)
{
    ...
    raft_index snapshot_index = logSnapshotIndex(
        &r->log);  // l->snapshot.last_index;//日志中被打快照的最后一个index？
    raft_index next_index =
        progressNextIndex(r, i);  // r->leader_state.progress[i].next_index;
...
static int triggerAll(struct raft *r)
{
    ...
    for (i = 0; i < r->configuration.n; i++) {
        ...
        //跳过RAFT_SPARE和正在从RAFT_SPARE变成RAFT_VOTER，LOGGER也会正常发
...
static int triggerActualPromotion(struct raft *r)
{
    ...
    printf("server %d change role %d to %d\n", server->id, old_role,
           RAFT_VOTER);
                      ...
                      const struct raft_append_entries_result *result)
{
    ...
    if (result->rejected > 0) {
        ...
        if (retry) {
            ...
            if (i == 3)
                printf("log mismatch -> send old entries to %llu\n",
                       server->id);
                                    ...
                                    const struct raft_append_entries *args)
{
    ...
    if (local_prev_term == 0) {
        if (r->id == 4)
            printf("no entry at index %llu -> reject\n", args->prev_log_index);
        return 1;
    }
...
int loggerReplicationAppend(struct raft *r,
                            const struct raft_append_entries *args,
                            raft_index *rejected,
                            bool *async)
{
    ...
    *async = true;
    ...
    printf("loggerReplicationAppend done\n");
    return 0;
...
// TODO 分析logger该如何改写replicationAppend方法
                      ...
                      bool *async)
{
    ...
    *rejected = 0;
    ...
    n = args->n_entries - i; /* Number of new entries */
    // i初始化为 0 deleteConflictingEntries 会对其进行修改
...
static void applyChange(struct raft *r, const raft_index index)
{
    ...
    if (r->configuration_uncommitted_index == index) {
        //重置未提交索引！
        struct raft_server *server;
        printf("server %d uncommitted_index to 0\n", r->id);
        printf("server %d configuration\n", r->id);
        for (int i = 0; i < r->configuration.n; i++) {
            printf("server %d, role %d, address %s\n",
                   r->configuration.servers[i].id,
                   r->configuration.servers[i].role,
                   r->configuration.servers[i].address);
        }
        r->configuration_uncommitted_index = 0;
    }
    // configuration_index也会被持久化，所以logger必须接收最新的configuration_index？
    r->configuration_index = index;
    //为什么只有leader才能进行操作，因为只有当一个server为leader的时候，他才会使用到configuration
    if (r->state == RAFT_LEADER) {
        const struct raft_server *server;
        ...
        if (server == NULL || server->role != RAFT_VOTER) {
            printf("leader removed from config or no longer voter server: %p\n",
                   (void *)server);
            convertToFollower(r);
        }
        // TODO 暂时不能理解？不加上这段 raft_add就正常了。
        //         if (req != NULL && req->cb != NULL) {
        //             printf("call req->cb ***\n");
        //             abort();
        //             req->cb(req, 0);
