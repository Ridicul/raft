 ...
 *
 * TODO: we should probably set configuration_uncommitted_index as well, since
...
int raft_start(struct raft *r)
{
    ...
    printf(
        "server:%lld state:%d current_term:%llu voted_for:%llu "
        "start_index:%llu n_entries:%zu\n",
        r->id, r->state, r->current_term, r->voted_for, start_index, n_entries);
     ...
     * configuration. */
    printf("restore %zu entries starting at %llu\n", n_entries, start_index);
