...
static void requestVoteSendCb(struct raft_io_send *req, int status)
    ...
    //    printf("self:%llu from:%llu@%s candidate_id:%llu disrupt_leader:%d last_log_index:%llu "
    ...
    if(server == NULL){
        printf("server %d 的configuration中无id为%d的配置\n",r->id,id);
        //直接返回，不做任何处理
        return 0;
    }

    if(server->role == RAFT_VOTER)
        printf("request vote from server %d role %d\n",server->id, server->role);
