...
static void recvSendAppendEntriesResultCb(struct raft_io_send *req, int status)
    ...
    //match 为 1的含义？recvEnsureMatchingTerms调recvCheckMatchingTerms，返回1，说明logger的term小于leader的term
    //第一次接收到entry的时候，说明
    //可以在这里给改，主要是扩容
    printf("ensure match %d\n",match);
    printf("logger log offset %d, refs_size %d front %d back %d size %d\n"
           ,r->log.offset, r->log.refs_size, r->log.front, r->log.back, r->log.size);
    ...
    printf("logger follower_state id %d, address %s\n",
           r->follower_state.current_leader.id, r->follower_state.current_leader.address);
    if (rv != 0) {
        return rv;
    }
    ...
    printf("recvLoggerAppendEntries: %5u\n",syscall(SYS_gettid));
...
int recvAppendEntries(struct raft *r,
    ...
    const struct raft_append_entries *args)
        {
    if(r->id == 4){
        printf("logger recv args n_entries %d, term %d, prev_log_index %d, leader_commit %d, prev_log_term %d\n"
               ,args->n_entries, args->term, args->prev_log_index, args->leader_commit, args->prev_log_term);
    }
    struct raft_io_send *req;
    struct raft_message message;
    struct raft_append_entries_result *result = &message.append_entries_result;
