...
static void initProgressLogger(struct raft_progress *p,
                               struct raft_progress *leader,
                               raft_index last_index)
{
    // TODO 怎么配置？可以让logger不用接收leader的snapshot，只接收entries。
    p->next_index = leader->match_index + 1;
    p->match_index = leader->match_index;
    p->snapshot_index = 0;
    p->last_send = 0;
    p->snapshot_last_send = 0;
    p->recent_recv = false;
    p->state = PROGRESS__PROBE;  //这个什么作用呢？
}
...
int progressBuildArray(struct raft *r)
{
    ...
    printf("progressBuildArray get timestamp %lld\n", now);
    ...
    for (i = 0; i < r->configuration.n; i++) {
        ...
        progress[i].last_recv_time = now;
    }
    printf("server %d call progressBuildArray %d\n", r->id, r->configuration.n);
                         ...
                         raft_index index)
{
    ...
    for (i = 0; i < configuration->n; i++) {
        ...
        if (id == r->id) {  //获取leader本身的progress
            leaderIndex = i;
            continue;
        }
        ...
        //新加入集群的server
        if (configuration->servers[i].role == RAFT_LOGGER) {
            //获取offset
            initProgressLogger(&progress[i], &progress[leaderIndex],
                               r->log.offset);
        } else
            initProgress(&progress[i], last_index);
    }
