...
static int uvClientSend(struct uvClient *c, struct uvSend *send)
{
    ...
    //这个函数就是发出了，那么接收的地方呢？recv_request_vote.c?
...
//发送不同类型 RPC 其中raft_message就是携带的内容
// vote&reply，appendEntries&reply，snapshot，timeout
           ...
           raft_io_send_cb cb)
{
    ...
    //    client包含了目标服务器的id和address，send->req里面的data是struct
    //    raft是发送者的信息
    rv = uvClientSend(client, send);
    if (rv != 0) {
        printf("server %d uvClientSend error\n", message->server_id);
        goto err_after_send_alloc;
    }
