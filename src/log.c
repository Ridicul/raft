...
// TODO 重写logAppend
int loggerLogAppend(struct raft_log *l,
                    const raft_term term,
                    const raft_index index,
                    ...
                    void *batch)
{
    ...
    if (l->size == 2 && l->back == 1) {  // TODO 说明这是第一个加入logger的entry
        ...
        //完成初始化的扩容之后，就需要开始 size和back的赋值了
        /* Release the old entries array. */
        if (l->entries != NULL) {
            raft_free(l->entries);
        }
...
static void removePrefix(struct raft_log *l, const raft_index index)
{
    ...
    printf(
        "removePrefix log info: size %d, front %d, back %d, offset %d, "
        "refs_size %d\n",
        l->size, l->front, l->back, l->offset, l->refs_size);
    clearIfEmpty(l);
}
...
void logSnapshot(struct raft_log *l, raft_index last_index, unsigned trailing)
{
    ...
    printf("last_index %d, trailing %d\n", last_index, trailing);
