...
{
    if (req != NULL && req->cb != NULL) {
        printf("convertFailChange call\n");
        req->cb(req, RAFT_LEADERSHIPLOST);
        printf("convertFailChange done\n");
    }
}
...
{
    printf("server %d convertToFollower\n", r->id);
    convertClear(r);
...
int convertToCandidate(struct raft *r, bool disrupt_leader)
{
    ...
    // TODO 如果disrupt_leader true，那么当前的candidate处于in_pre_vote状态。
    ...
    printf("server %d state %d\n", r->id, server->role);
    assert(server != NULL);
    assert(server->role == RAFT_VOTER);
    //如果只有一个投票者且是自己，那就快进到leader
    if (n_voters == 1) {
...
{
    printf("server %d convertToLeader\n", r->id);
    int rv;
