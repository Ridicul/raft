...
int membershipCanChangeConfiguration(struct raft *r)
{
    ...
    //现在还有一个更改配置的请求尚未提交，所以不能加入集群。
    if (r->configuration_uncommitted_index != 0) {
        printf("server %d r->configuration_uncommitted_index %llu\n", r->id,
               r->configuration_uncommitted_index);
        rv = RAFT_CANTCHANGE;
        goto err;
    }

    //证明现在有一个转换成voter的spare
    //server，raft规定不能同时有两个server加入集群，所以RAFT_CANTCHANGE。
    if (r->leader_state.promotee_id != 0) {
        printf("r->leader_state.promotee_id %llu\n",
                                ...
                                const struct raft_entry *entry)
{
    ...
    printf("server %d configuration_uncommitted_index %d change to %d\n", r->id,
