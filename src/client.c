...
int raft_add_logger(struct raft *r,
                    struct raft_change *req,
                    raft_id id,
                    const char *address,
                    raft_change_cb cb)
{
    ...
    rv = raft_configuration_add(&configuration, id, address, RAFT_LOGGER);
    //此时 临时的configuration已经有具体的RaftLogger了
    if (rv != 0) {
             ...
             raft_change_cb cb)
{
    ...
    printf("add server: id %llu, address %s\n", id, address);
    ...
    assert(r->leader_state.change == NULL);
    r->leader_state.change = req;
    //    printf("leader_state_change      ");
    //    r->leader_state.change->cb(NULL,0);
    return 0;
                ...
                raft_change_cb cb)
{
    ...
    printf("raft_assign to id:%llu the role:%d\n", id, role);
    ...
    printf("raft_assign replicationProgress server_index %d\n", server_index);
