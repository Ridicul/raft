...
static int electionSend(struct raft *r, const struct raft_server *server)
{
    ...
    // TODO 发送者的信息（struct raft）包装在raft_io_send的data中？就离谱
    send->data = r;

    rv = r->io->send(r->io, send, &message, sendRequestVoteCb);
    // send func impl in uv_send.c UvSend
    if (rv != 0) {
        RaftHeapFree(send);
        return rv;
    }
...
int electionStart(struct raft *r)
{
    ...
    //    printf("candidate %d start election\n", r->id);
    for (i = 0; i < r->configuration.n; i++) {
        const struct raft_server *server = &r->configuration.servers[i];
        //发送给除自己外和角色是RAFT_VOTER的server
        if (server->id == r->id || server->role != RAFT_VOTER) {
            //            printf("xxxxx server %d cant send election to server
            //            %d role %d\n",r->id, server->id, server->role);
            continue;
        }
        //        printf("√√√√√ server %d send election to server %d role
        //        %d\n",r->id, server->id, server->role);
...
grant_vote:
    ...
    tracef("server %d vote granted to %llu", r->id, args->candidate_id);
    *granted = true;
